...
public class ExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    public ExpandableListAdapter(Context context, List<OrderItemModel> listDataHeader, HashMap<String, List<String>> listDataChild, OnItemCount onItemCount) {
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = infalInflater.inflate(R.layout.selected_item_view, null);
        ...
        holder.llMinus.setOnClickListener(v -> {
            if (groupPosition != -1) {
                ...
                new Thread(() -> {
                    count = Integer.parseInt(orderItemModel.getCountPrice()) - 1;
                    if (count == 0) {
                        getDataManager().updatecount(("1"), orderItemModel.getItemId());
                        orderGetCount = getDataManager().loadCount(orderItemModel.getItemId());
                    } else {
                        getDataManager().updatecount(String.valueOf(count), orderItemModel.getItemId());
                        orderGetCount = getDataManager().loadCount(orderItemModel.getItemId());
